...
void UAIComponent::SaveEntityState() const
{
	...
	{
		UE_LOG(LogTemp, Log, TEXT("[UAIComponent::SaveEntityState]: Game saved successfully on slot: %s"), *EntityChecksum);
	}
}

void UAIComponent::LoadEntityState()
{
	UE_LOG(LogTemp, Log, TEXT("[UAIComponent::LoadEntityState]: Try loading data for slot = %s"), *EntityChecksum);
